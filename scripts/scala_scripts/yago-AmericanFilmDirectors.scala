...
def sparqlQuery(endpoint: String): URL = {
  def url(q: String) = endpoint + "?query=" + q + "&format=text/csv&timeout=10000000"
  val query = encodeHtmlEntities("""
    SELECT DISTINCT iri(concat(replace(str(?uri),"http://dbpedia.org/resource/","http://dbpedia.org/data/","i"), ".rdf")) AS ?americanDirectors
    WHERE { 
      ?uri a yago:AmericanFilmDirectors
    }
    LIMIT 10000
    OFFSET 0
  """)
