...
public class SmartAnalyzingSuggester extends Lookup {
    ...
    try {
      ...
      while ((surfaceForm = iterator.next()) != null) {
        ...
        for (IntsRef path : paths) {
          ...
          if (hasPayloads) {
            ...
            requiredLength += 2 + curentContainingDocs.length * 4;
          }
      ...
      while (reader.read(scratch)) {
        ...
        if (!hasPayloads) {
          ...
          BytesRef br = new BytesRef(surface.length + 1 + containingDocsLength);
  ...
  private LookupResult getLookupResult(Long output1, BytesRef output2, CharsRefBuilder spare) {
    ...
    if (hasPayloads) {
      ...
      final int containingDocsLength = output2.length - sepIndex - 1; 
